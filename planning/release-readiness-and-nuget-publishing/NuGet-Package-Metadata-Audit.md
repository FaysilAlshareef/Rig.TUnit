# NuGet Package Metadata Audit

Per-`csproj` table for the metadata sweep in **Phase C** of the release roadmap. Anything
the root `Directory.Build.props` provides (Authors, RepositoryUrl, License, ReadmeFile, Icon,
Tags) is **not** repeated here — only `<Description>`, which is package-specific.

---

## Existing descriptions (do not change)

These two already have a `<Description>` — preserve as-is:

| Project | Current description |
|---|---|
| `src/Rig.TUnit/Rig.TUnit.csproj` | "Convenience meta-package bundling Core + Mediator + Grpc + WebAPI — the default entry point for most projects. Use Rig.TUnit.All only when you need every package." |
| `src/Rig.TUnit.All/Rig.TUnit.All.csproj` | "Meta-package containing every Rig.TUnit.* package. DISCOURAGED — prefer per-feature or per-stack meta-packages (Rig.TUnit, Rig.TUnit.Microservices)." |

---

## Descriptions to add (one per project)

Family base packages depend on per-provider leaf packages; descriptions reflect that.
...
| `Rig.TUnit.Databases.Sql.SqlServer` | TUnit fixture for SQL Server backed by Testcontainers. Spin up an isolated `mssql` container per test with a one-line `new SqlServerFixture()` and assert against it via Dapper or EF Core. |
...
| `Rig.TUnit.Databases.NoSql.Redis` | TUnit fixture for Redis backed by Testcontainers. Use for Redis-as-database scenarios; for caching see `Rig.TUnit.Caching.Redis`. |
...
| `Rig.TUnit.Messaging.ServiceBus` | TUnit fixture for Azure Service Bus backed by Testcontainers. Native sessions, deduplication, partition keys, and runtime topology builder for topics + subscriptions + rules. |
...
| `Rig.TUnit.Microservices.EventSourcing` | TUnit fixture for event-sourced aggregates — RED/GREEN given/when/then helpers, stream replay, and projection drift assertions. |
...
## Audit script (run during Phase C)

```bash
# List every src csproj missing <Description>
...
Cross-reference the output with the table above and apply the corresponding `<Description>`
inside the existing `<PropertyGroup>` (or add a new one).

---

## Per-csproj patch template

```xml
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <Description>{{ from table above }}</Description>
    <PackageTags>$(PackageTags);{{ optional, project-specific extras e.g. postgres;dapper }}</PackageTags>
  </PropertyGroup>
  <!-- existing ItemGroups unchanged -->
</Project>
```

Provider-specific tag additions (appended to the inherited base list):

| Family | Extra tags |
|---|---|
| SQL | `sql;{provider}` (e.g. `sql;postgresql`) |
...
```bash
dotnet pack Rig.TUnit.slnx -c Release -o ./artifacts
# Expected: zero NU5* warnings.

# Inspect any produced nupkg:
unzip -p artifacts/Rig.TUnit.Core.0.0.0-alpha.0.{height}.{sha}.nupkg \
  Rig.TUnit.Core.nuspec | xmllint --xpath \
    'string(/*[local-name()="package"]/*[local-name()="metadata"]/*[local-name()="description"])' -
```

The `pack-validate` CI job (Phase E-9) automates this for every PR.
