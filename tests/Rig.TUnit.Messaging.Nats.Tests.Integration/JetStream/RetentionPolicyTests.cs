...
public sealed class RetentionPolicyTests
{
    ...
    public async Task Stream_WithMaxMsgs10_DropsOldestWhenLimitExceeded(CancellationToken ct)
    {
        ...
        await fx.EnsureStreamAsync(streamName, [subject], maxMsgs: 10, ct: ct);
        ...
        // Allow the broker a moment to apply retention.
        await Task.Delay(500, ct);

        // Assert — stream reports at most 10 messages stored.
        // StreamState.Messages is `long` (signed) in NATS.Client.JetStream 2.5,
        // not `ulong`. Use a long literal so type inference picks up the
        // ValueAssertion<long> overload.
        var stream = await fx.GetStreamAsync(streamName, ct);
        await Assert.That(stream.Info.State.Messages).IsLessThanOrEqualTo(10L);
    }
}
