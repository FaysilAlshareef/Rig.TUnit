...
**TDD discipline — UNSKIPPABLE (added 2026-04-18 after mid-feature drift)**

- **FR-030**: Every provider MUST ship tests in all four categories — **Unit**, **Integration**, **Contract**, **Benchmark**. A provider is NOT "canonical" until all four test types are present. See tasks.md §TDD Gate for the per-category project path pattern.
- **FR-031**: Every task touching `src/` MUST be committed as TWO commits in this order: `test(004): TNNN — RED for {Type}` (all new test files, run confirmed failing) followed by `feat(004): TNNN — GREEN implement {Type}` (production code; newly-added tests now GREEN). `TestCompletenessTests` (added in T157a) enforces the structural requirement at architecture-test level; the commit-order requirement is enforced by T171 at PR gate.
- **FR-032**: Every `src/Rig.TUnit.{Family}.{Provider}/` MUST have a matching `tests/Rig.TUnit.{Family}.{Provider}.Tests.Unit/` project. Per-provider unit tests cover pure-function helpers, Options validation, RigBuilder wiring, and `Use{Provider}` fluent entry — none of which require Docker.
- **FR-033**: Every provider MUST contribute at least one BenchmarkDotNet class to `tests/Rig.TUnit.Benchmarks/` measuring allocation + throughput of its public surface. Benchmarks run under `dotnet run -c Release --project tests/Rig.TUnit.Benchmarks` and a PR regression > 20 % vs the Phase-3 baseline MUST be root-caused before merge.
- **FR-034**: Retroactive TDD remediation for commits that landed src code without preceding RED (the Phase 3.0 Postgres commit `2b149b2` is the sole known violation at the time of writing — tracked as task T176a) MUST land before any further provider work resumes. Going forward, CI blocks any PR whose `git log` does not show a RED commit preceding every GREEN commit on the feature branch.

- **FR-035 (coverage-lifting tests)**: Every provider MUST ship two additional unit tests on top of the canonical four categories, because first-pass measurement on Mongo and Postgres (2026-04-18) showed the basic template lands at 77–87 % line coverage — short of the 90 % gate. The two mandatory additions are:
  1. `{Provider}FixtureOptionsTests.cs` — explicitly constructs the Options record with defaults and with every property overridden. Required because init-only autoprops do not register as line-covered when exercised only through DI binding (a coverlet measurement quirk verified empirically).
  2. `{Provider}RigBuilder_ExerciseTests.cs` (or `{Provider}RigBuilderConnectionStringTests.cs` for NoSql-style providers) — drives `UseProvider` (SQL) or the `ConnectionString` getter (NoSql/Messaging/Caching/Storage/Security/Observability) through a minimal test double. Covers the remaining RigBuilder line paths that are otherwise only hit by downstream integration.
  Acceptance: after both tests land alongside the canonical four, merged unit + integration coverage via `dotnet run -- --coverage --coverage-output-format cobertura` is ≥ 90 % line / ≥ 85 % branch per package.

- **FR-036 (coverage measurement mechanism)**: TUnit test projects run under Microsoft.Testing.Platform (MTP), not VSTest. `coverlet.msbuild` integration through `/p:CollectCoverage=true` is NOT supported under MTP. Coverage MUST be collected via the TUnit/MTP-native flag: `dotnet run --no-build -c Debug --project tests/<proj>/ -- --coverage --coverage-output-format cobertura --coverage-output <name>.cobertura.xml`. Output lands in `<proj>/bin/Debug/net10.0/TestResults/`. The `coverlet.collector` + `coverlet.msbuild` packages pinned in `Directory.Packages.props` (T002) remain as transitive-graph insurance but are not the primary collection path.
